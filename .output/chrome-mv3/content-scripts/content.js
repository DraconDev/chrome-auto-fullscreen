  ...
  const definition = defineContentScript({
    ...
    async main() {
      ...
      try {
        ...
        if (stored == null ? void 0 : stored[MMB_KEY]) {
          newTabIntent = true;
        }
      } catch {
      }
      document.addEventListener(
        ...
        (e) => {
          if (e.ctrlKey || e.metaKey || e.altKey || e.button === 1) {
            newTabIntent = true;
            browser$1.storage.local.set({ [MMB_KEY]: true }).catch(() => {
            });
            browser$1.runtime.sendMessage({ action: "setModifiers", ctrl: true });
          }
        },

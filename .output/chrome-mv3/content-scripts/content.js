  ...
  const definition = defineContentScript({
    ...
    async main() {
      ...
      try {
        ...
        if (testEvent.getModifierState("Control") || testEvent.getModifierState("Meta") || testEvent.getModifierState("Alt")) {
          newTabIntent = true;
        }
      } catch {
      }
      ...
      document.addEventListener(
        ...
        (e) => {
          if (e.ctrlKey || e.metaKey || e.altKey || e.button === 1) {
            newTabIntent = true;
            browser$1.storage.local.set({ [MMB_KEY]: { url: location.href, ts: Date.now() } }).catch(() => {
            });
            browser$1.runtime.sendMessage({ action: "setModifiers", ctrl: true });
          }
        },
